	...
	{:else if $recipes.length > 0}
		...
		<ul>
			...
			{#each $recipes[0].ingredients as ingredient}
				<li class="ml-4 list-disc">{ingredient.name}</li>
			{/each}
		</ul>
		<ul>
			Catégories :
			{#each $recipes[0].categories as category}
				<li class="ml-4 list-disc">{category.name}</li>
			{/each}
		</ul>
		<div class="mt-4 flex gap-3">
			<button
				class="w-30 rounded bg-indigo-600 px-4 py-2 font-medium text-white hover:bg-indigo-700"
				on:click={() => (window.location.href = `/recipe/${$recipes[0].id}/update`)}
				>Modifier</button
			>
			<button
